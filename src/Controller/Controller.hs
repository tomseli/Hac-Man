...
step dt state = do
  ...
  checkStatus state { elapsedTime = elapsedTime state + dt, player = updatedPlayer }
    where
      ...
      -- updatedPlayer =
      --   let newMovement = ((movement.entity.player) state) { deltaTime = elapsedTime state + secs }
      --       newEntity = ((entity.player) state) { movement = newMovement }
      --   in (player state) { entity = moveStep newEntity ((speed.movement) newEntity) }
