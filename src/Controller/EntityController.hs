...
moveWithCollision ent totalMovement maze = helperFunction ent steps
    where 
      ...
      helperFunction entity n =        --recursive helper function
...
actOnCollision ent (MkWall _)                 = Just $ changeDirEnt (ent{movement = (movement ent){position =  snapToGrid ((position.movement) ent)}}) Still
...
checkValidHeading ent = undefined
...
testEntity =
  MkEntity
    { movement =
        MkMovement{direction = Model.Entities.Still, speed = 1000, position = (1, -1), heading = Still}
    , alive = Alive
    }
